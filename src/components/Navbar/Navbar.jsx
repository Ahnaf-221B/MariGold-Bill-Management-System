...
const Navbar = () => {
  ...
  return (
    <header className="bg-blue-50 shadow-md py-4 px-6 sm:px-10 md:px-20 flex justify-between items-center relative">
      ...
      <nav className={`flex flex-col space-x-6 text-gray-700 font-medium ${mobileMenuOpen ? 'block' : 'hidden'} sm:block`}>
        <Link to="/" className="hover:text-blue-600 py-2 px-3">Home</Link>
        <Link to="/bills" className="hover:text-blue-600 py-2 px-3">Bills</Link>
        <Link to="/profile" className="hover:text-blue-600 py-2 px-3">My Profile</Link>
      </nav>
      ...
      <div className="space-x-4 flex items-center">
        {!user ? (
          ...
          // If the user is logged in, show the Profile icon and dropdown
          <>
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 bg-white shadow-lg rounded-lg w-60 h-auto z-50">
                  <div className="py-2 px-4 text-sm text-gray-700">
                    <div className="font-semibold text-lg">Current Balance: ${balance}</div>
                    <button
